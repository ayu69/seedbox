...
echo -e "${CCYAN}INSTALLATION${CEND}"
	...
	case $PORT_CHOICE in
		...
		2) 	## Mise en place des variables necéssaire au docker-compose
			...
			echo -e "${CGREEN}															  ${CEND}"
			echo -e "${CCYAN}-------------------------------------------------------------------------------------------------------------------------${CEND}"
			echo -e "${CCYAN}	  ${CPURPLE}VOLUMES_ROOT_PATH:${CRED} Emplacement des volumes sur l'hote:	${CCYAN}/home/dirtygeek	  ${CEND}"
			echo -e "${CCYAN}-------------------------------------------------------------------------------------------------------------------------${CEND}"
			echo -e "${CGREEN}															  ${CEND}"
			...
			export VOLUMES_ROOT_PATH=/home/dirtygeek
			...
			read -rp "Voulez modifier la variable ci dessus ? (o/n) : " EXCLUDE
				if [[ "$EXCLUDE" = "o" ]] || [[ "$EXCLUDE" = "O" ]]; then
					...
					echo -e "${CGREEN}${CEND}"
					echo -e "${CCYAN}Par défault le montage des volumes est dans : /home/dirtygeek ${CEND}"
					read -rp "VOLUMES_ROOT_PATH = " VOLUMES_ROOT_PATH
						...
						then
			 				...
			 				VOLUMES_ROOT_PATH=/home/dirtygeek
			 				export VOLUMES_ROOT_PATH
						fi
				fi
			...
			echo -e "${CRED}	${CCYAN}OMBI_FQDN:${CRED}		ombi.${DOMAIN}							  ${CEND}"
			echo -e "${CRED}-------------------------------------------------------------------------------------------------------------------------${CEND}"
			...
			services:
			  ...
			  ombi:
			    ...
			    volumes:
			      ...
			      - ${VOLUMES_ROOT_PATH}/ombi/shared:/shared
			    ports:
			      - "3579:3579"
			    environment:
			      - PUID=0
		...
		3)
			...
			echo -e "${CGREEN}   13) Ombi ${CEND}"
			echo -e "${CGREEN}   14) Retour Menu Principal ${CEND}"
			echo ""
			read -p "Appli choix [1-14]: " -e -i 1 APPLI
			echo ""			
			case $APPLI in
				1)
				...
				if docker ps -a | grep -q ombi; then
					...
					docker-compose up -d ombi 2>/dev/null
					progress-bar 20
					echo ""
